...
class LoginForm(form.Form):
    ...
    def validate_login(self, field):
        ...
        print("user password :", user.password, "  password.data ", self.password.data)
        # we're comparing the plaintext pw with the the hash from the db
        if not check_password_hash(user.password, self.password.data):  		# plaintext passwd != hashed passwd , commented 3.4.
        # to compare plain text passwords use
        # if user.password != self.password.data:
            print('raising not check_password_hash error')
...
def scanned(id):
    ...
    return render_template('/admin/userScanned.html',
                           title='Sign In',
                           form=form)
